...
export default function SalesList() {
  ...
  const handleUndo = async (order: SalesOrder) => {
    ...
    // Reverse each item: add back stock, restore batches
    for (const item of order.items) {
      // Find the oldest batches for this product and restore (approximate reversal)
      const productBatches = updatedBatches.filter(b => b.productId === item.productId)
      let remaining = item.quantity
      // First try to add back to batches that might have been the source
      ...
      // If no matching batch found, create a reversal batch
      if (remaining > 0 || productBatches.length === 0) {
        updatedBatches.push({
          id: generateId(), productId: item.productId, productName: item.productName,
          batchNo: 'REV-' + order.orderNo,
          quantity: item.quantity, costPrice: item.costPrice,
          purchaseOrderId: 'reversal', receivedAt: Date.now(),
        })
      }
      ...
      logs.push({
        ...
        quantity: item.quantity, batchNo: '撤销-' + order.orderNo,
        relatedOrderId: order.id, relatedOrderType: 'sales',
        createdAt: Date.now(),
      })
    }
  ...
  return (
    <div>
      ...
      {orders.length === 0 ? (
        ...
        <div className="card"><table className="data-table"><thead><tr><th>单号</th><th>商品数</th><th>销售额</th><th>毛利</th><th>日期</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>{orders.map(o => (
            <tr key={o.id}>
